...
def does_branch_exist(digi_name, branch_name):
    response = os.popen(f"curl -s -X GET -H 'Accept: application/json' http://localhost:9867/pool/{digi_name}/branch/{branch_name}").read()
    ...
    if spec is None:
        ...
        if mount is not None:
            for child_group in mount.keys():
                child_group_dict = mount.get(child_group)
                for child in child_group_dict.keys():
                    child_hash = os.popen(f"digi checksum {get_child_digi_name(child, get_namespace(None))}").read()
            ...
            if os.path.exists(children_path):
                for item in os.scandir(children_path):
                    if os.path.isdir(item.path):
                        child_hash = os.popen(f"digi checksum {dirname}/children/{os.path.basename(item.path)}").read()
                        hash_list.append(child_hash)
    rf.close()
...
if __name__ == '__main__':
    if (len(sys.argv) > 2):
        if sys.argv[1] == "save-branches":
            ...
            find_kind(sys.argv)
        elif sys.argv[1] == "hier-checksum-digi":
            hier_checksum_digi(sys.argv)
        elif sys.argv[1] == "hier-checksum-snapshot":
